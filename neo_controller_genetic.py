...
class GeneticNeoController(KesslerController):
    ...
    def finish_init(self, game_state, chromosome):
        ...
        theta_delta['NL'] = fuzz.zmf(theta_delta.universe, chromosome[4].value, chromosome[5].value)
        theta_delta['NS'] = fuzz.trimf(theta_delta.universe, chromosome[6].value)
        ...
        ship_turn['NL'] = fuzz.trimf(ship_turn.universe, chromosome[11].value)
        ship_turn['NS'] = fuzz.trimf(ship_turn.universe, chromosome[12].value)
        ...
        ship_thrust['NL'] = fuzz.trimf(ship_thrust.universe, chromosome[18].value)
        ship_thrust['NS'] = fuzz.trimf(ship_thrust.universe, chromosome[19].value)
    ...
    def actions(self, ship_state: Dict, game_state: Dict) -> Tuple[float, float, bool]:
        if not self.init_done:
            self.finish_init(game_state, chromosome=self.chromosome)
            self.init_done = True
        #print('Self:')
        #print(self)
        #print('Game state:')
